...
function acessarAbaVendedores() {
    ...
    const admin = vendedores.find(v => v.nome === nome && v.senha === senha && v.isAdmin);
...
function renderProducts(productsArray) {
    ...
    productsArray.forEach(produto => {
        ...
        div.innerHTML = `
            ...
            ${produto.maxDesconto ? `<p>Desc. Máx: ${produto.maxDesconto}%</p>` : ''}
...
function updatePaymentUI() {
    ...
    if (selectedPaymentMethods.length > 1) {
        ...
        trocoSection.style.display = selectedPaymentMethods.includes('dinheiro') ? 'block' : 'none';
    }
}
...
function renderPaymentSplit() {
    ...
    selectedPaymentMethods.forEach((method, index) => {
        ...
        div.innerHTML = `
            ${method.toUpperCase()}: 
            <input type="number" id="valor-${method}" 
                placeholder="Valor ${method}" 
                oninput="calculateSplitPayment()">
        `;
        container.appendChild(div);
    });
}
...
function calculateSplitPayment() {
    ...
    selectedPaymentMethods.forEach(method => {
        const value = parseFloat(document.getElementById(`valor-${method}`).value) || 0;
        sum += value;
    });
    ...
    vendedores.forEach(vendedor => {
        ...
        div.innerHTML = `
            ...
            <div class="vendedor-actions">
                <button onclick="editarVendedor('${vendedor.nome}')">Editar</button>
                <button onclick="alternarStatusVendedor('${vendedor.nome}')">
                    ${vendedor.ativo ? 'Desativar' : 'Ativar'}
                </button>
                ${!vendedor.isAdmin ? `<button onclick="excluirVendedor('${vendedor.nome}')">Excluir</button>` : ''}
            </div>
        `;
        lista.appendChild(div);
    });
    ...
    vendas.filter(venda => {
        ...
        html += `
            <div class="item-carrinho">
                ...
                <div>Produtos: ${venda.itens.map(item => `${item.nome} (${item.quantidade}x)`).join(', ')}</div>
